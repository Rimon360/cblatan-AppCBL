...
const Assign_shop = () => {
  ...
  useEffect(() => {
    const getAssignedId = shops.filter((shop) => {
      if (shop.isAssigned)
        return shop._id;
    }).map((shop) => shop._id);
  ...
  return (
    <>
      <section className="shop-section p-6">
        ...
        <div>
          <br />
          <button onClick={() => handleAssigningAll(shops)} className={` pl-2 pr-2 pt-1 pb-1 rounded-lg cursor-pointer mb-2  ${isAssignedAll ? "bg-red-100 text-red-500 hover:bg-red-500 hover:text-white" : "bg-green-100 text-green-500 hover:bg-green-500 hover:text-white"}`}>
