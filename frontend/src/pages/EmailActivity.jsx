...
const EmailActivity = () => {
  ...
  useEffect(() => {
    axios
      .get(BACKEND_URL + "/api/email/email_activity/get", { headers: { Authorization: "Bearer " + token } })
      .then((response) => {
        setHistory(response.data?.email_activity || [])
        setFilteredHistory(
          users.filter(
            (history) =>
              history?.email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
              history?.email_body?.toLowerCase().includes(searchTerm.toLowerCase()) ||
              history?.email_title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
