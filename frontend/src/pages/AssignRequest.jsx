...
const AssignRequest = () => {
  ...
  useEffect(() => {
    axios
      .get(BACKEND_URL + "/api/shops/get-assign-requested", { headers: { Authorization: "Bearer " + token } })
      .then((response) => { 
        setHistory(response.data?.shops || [])
        setFilteredHistory(
          users.filter(
            (history) =>
              history?.course_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
              history?.requested_by_email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
              history?.requested_for_email?.toLowerCase().includes(searchTerm.toLowerCase()),
          ),
        )
      })
      .catch((error) => {
  ...
  const handleDeleteHistory = async (id) => {
    try {
      let res = await axios.post(BACKEND_URL + "/api/shops/delete-assign-requested", { id }, { headers: { Authorization: "Bearer " + token } })
      setRefresh((prev) => !prev)
  ...
  const handleApproveHistory = async (id) => {
    try {
      let res = await axios.post(BACKEND_URL + "/api/shops/approve-assign-requested", { id }, { headers: { Authorization: "Bearer " + token } })
      setRefresh((prev) => !prev)
      toast.success(res.data.message)
    } catch (error) {
      toast.error(error.message)
            ...
            <table className="w-full max-h-300 overflow-auto mt-4 text-left border-collapse">
              <thead>
                <tr className="bg-gray-200">
                  <th className="p-2">#</th>
                  <th className="p-2">course_name</th>
                  <th className="p-2">requested_by_email</th>
                  <th className="p-2">requested_for_email</th>
                  ...
                  filteredHistory.map((history, i) => {
                    return (
                      <tr key={history._id} className="hover:bg-gray-100">
                        ...
                        <td className="p-2 text-green-400 select-all">{history.course_name}</td>
                        <td className="p-2">{history.requested_by_email || "N/A"}</td>

                        <td>
                          <div className="!max-w-[300px] !max-h-[100px] overflow-auto  break-words">{history.requested_for_email || "N/A"}</div>
                        </td>
                        <td>
