  ...
  if (!fs.existsSync(filePath)) {
    await new Promise((rs) => {
      ...
      let int = setInterval(() => {
        ...
        if (count > 3 && !fs.existsSync(filePath)) {
          clearInterval(int)
          res.status(200).json({ error: true, message: "Profile not found" })
        }
        count++
      }, 500)
    })
...
router.get("/ads/get", memberMiddleware, async (req, res) => {
  ...
  try {
    // let { role } = req.user
    let { ads_location } = req.query || {}
    if (!ads_location) {
      result = await AdsModel.find()
    } else {
      result = await AdsModel.find({ ads_location })
    }
    url = result[getRandomInRange(0, result.length - 1)]
  } catch (error) {
    res.status(403).json({ error: error.message, message: error.message })
  }
  res.status(200).json({ message: "success", url, ads: result })
})
